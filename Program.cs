...
class Program
{
    ...
    static string TargetManifestUrl => _remoteConfig?.ManifestUrl ?? FallbackManifestUrl;
    ...
    static async Task<int> Main(string[] args)
    {
        ...
        bool tooNew = currentVersion != null
            && long.TryParse(currentVersion, out var curVer)
            && long.TryParse(TargetVersion, out var tgtVer)
            && curVer > tgtVer;
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case "0":
                    return 0;
                default:
                    Console.WriteLine("Invalid option. Please enter 0-3.");
                    Console.WriteLine();
                    break;
            }
        }
    ...
    {
        string? currentVersion = GetSteamVersion(steamPath);
        bool needsDowngrade = currentVersion != TargetVersion;
        var stStatus = CheckSteamToolsStatus(steamPath);
        bool needsSteamTools = stStatus != SteamToolsStatus.UpToDate;

        if (!needsDowngrade && !needsSteamTools)
        {
        ...
        if (needsDowngrade)
            await ApplyTargetVersion(steamPath, skipPrompt: true);
        if (needsSteamTools)
            await InstallSteamTools(steamPath, skipPrompt: true);
    }

    static async Task ApplyTargetVersion(string steamPath, bool skipPrompt = false)
    {
        string? currentVersion = GetSteamVersion(steamPath);

        bool isDowngrade = currentVersion != null
            && long.TryParse(currentVersion, out var cur)
            && long.TryParse(TargetVersion, out var tgt)
            && cur > tgt;

        string action = currentVersion == TargetVersion ? "reinstall"
        ...
        {
            if (action == "reinstall")
                Console.WriteLine($"Will reinstall Steam at version {TargetVersion}.");
            else
                Console.WriteLine($"Will {action} from {currentVersion} to {TargetVersion}.");
        }
        else
            Console.WriteLine($"Will apply target version {TargetVersion}.");
        ...
        bool manifestOk = await DownloadManifest(packageDir);
        if (!manifestOk)
        {
        ...
        List<string> urls = await FetchSources();
        if (urls.Count == 0)
        {
        ...
        if (newVersion != null)
        {
            ...
            if (newVersion == TargetVersion)
            {
                ...
                Console.WriteLine($"WARNING: Version is {newVersion}, expected {TargetVersion}. The {action} may not have fully applied.");
        }
        else
        {
    ...
    static async Task InstallSteamTools(string steamPath, bool skipPrompt = false)
    {
        ...
        bool dlOk = true;
        dlOk &= await DownloadFile(httpClient, SteamToolsDllUrl, xinputPath, "xinput1_4.dll");
        dlOk &= await DownloadFile(httpClient, SteamToolsDwmapiUrl, dwmapiPath, "dwmapi.dll");
    ...
    static string? ExtractVersionFromManifest(string manifestPath)
    {
        try
        {
    ...
    static List<string> ReadEmbeddedSources()
    {
    ...
    static async Task<List<string>> FetchSources()
    {
        string? sourcesUrl = _remoteConfig?.SourcesUrl;
        if (!string.IsNullOrEmpty(sourcesUrl))
        {
            try
            {
                using var http = new HttpClient { Timeout = TimeSpan.FromSeconds(30) };
                string raw = await http.GetStringAsync(sourcesUrl);
    ...
    static List<string> ParseSourceLines(string text)
    {
        var urls = new List<string>();
    ...
    static async Task<bool> DownloadManifest(string packageDir)
    {
        try
        {
            using var httpClient = new HttpClient();
            ...
            using var response = await httpClient.GetAsync(TargetManifestUrl);
            response.EnsureSuccessStatusCode();
