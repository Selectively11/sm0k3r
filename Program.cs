...
class Program
{
    ...
    static void ToggleSteamUpdates(string steamPath, bool currentlyBlocked)
    {
        string cfgPath = Path.Combine(steamPath, "steam.cfg");

        if (currentlyBlocked)
        {
            Console.WriteLine("=== Unblock Steam Updates ===");
            Console.WriteLine();
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine("WARNING: This will allow Steam to auto-update, which may break SteamTools!");
            Console.ResetColor();
            Console.WriteLine();
            Console.Write("Proceed? [y/N] ");
            ...
            try
            {
                if (File.Exists(cfgPath))
                {
                    File.Delete(cfgPath);
                    Console.ForegroundColor = ConsoleColor.Green;
                    Console.WriteLine("Steam updates are now UNBLOCKED.");
                    Console.ResetColor();
                }
                else
                {
                    Console.WriteLine("steam.cfg does not exist, updates are already unblocked.");
                }
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"ERROR: Could not delete steam.cfg: {ex.Message}");
            }
        }
        else
        {
            Console.WriteLine("=== Block Steam Updates ===");
            Console.WriteLine();
            WriteSteamCfg(steamPath);
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine("Steam updates are now BLOCKED.");
            Console.ResetColor();
        }
        Console.WriteLine();
    }
    ...
    static SteamToolsStatus CheckSteamToolsStatus(string steamPath)
    {
        ...
        // dwmapi not patched by CloudFix, so mismatch = outdated
        if (dwmapiHash != cfg.SteamToolsDwmapiSha256)
            return SteamToolsStatus.Outdated;

        if (IsCloudFixPatched(xinputPath))
            return SteamToolsStatus.UpToDateCloudFixed;

        return SteamToolsStatus.Outdated;
    }

    static bool IsCloudFixPatched(string dllPath)
    {
        // CloudFix patches:
        // 0x272F: E8 7C F5 FF FF -> B8 01 00 00 00 (return 1, skip download)
        // 0x28B5: 74 -> EB (jz -> jmp, skip hash check)
        
        try
        {
            byte[] data = File.ReadAllBytes(dllPath);
            
            // Verify SteamTools AES key is present
            byte[] aesKey = {
            ...
            if (!ContainsBytes(data, aesKey))
                return false;
            
            if (data.Length < 0x28B6)
                return false;
                
            bool patch1 = data[0x272F] == 0xB8 
                && data[0x2730] == 0x01 
                && data[0x2731] == 0x00 
                && data[0x2732] == 0x00 
                && data[0x2733] == 0x00;
            
            bool patch2 = data[0x28B5] == 0xEB;
    ...
    static bool ContainsBytes(byte[] data, byte[] needle)
    {
